...
export class InfraSS58_TrustedEntity {
  ...
  public async addVerifier(authorizerId, verifierDID = this.that.didModule.did) {
    ...
    // @ts-ignore
    const entityIds = new BTreeSet();
    entityIds.add(this.that.didToHexPk(verifierDID) as unknown as Codec)
    const entity = { authorizerId, entityIds };
  ...
  public async getIssuers(authorizerId, issuerId) {
    const resp = await this.that.api.query.trustedEntity.issuers(authorizerId, this.that.didToHexPk(issuerId));
    if (resp.isNone) {
  ...
  public async getVerifiers(authorizerId, verifierId) {
    const resp = await this.that.api.query.trustedEntity.verifiers(authorizerId, this.that.didToHexPk(verifierId));
    if (resp.isNone) {
  ...
  public addPolicyOwner(ownerDID?: string) {
    ...
    this.owners.push(this.that.didToHexPk(ownerDID))
  }
}
